...
if __name__ == '__main__':
    ...
    while (cap.isOpened()):
        ...
        background_removed = cv2.absdiff(cv2.cvtColor(cv2.medianBlur(
            frame, 7), cv2.COLOR_BGR2GRAY), cv2.medianBlur(background, 7))

        # Otsu's threshold to get approximate estimate of object
        _, approx_obj = cv2.threshold(
            background_removed, 0, 255, cv2.THRESH_OTSU)
        ...
        contours, hierarchy = cv2.findContours(
            approx_obj, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        sorted_contour = sorted(
            zip(area_contour, contours), key=lambda x: x[0], reverse=True)
        ...
        for i in range(0, obj):
            if i < len(sorted_contour):
                ...
                cv2.circle(frame, (int(cpx[0]), int(
                    cpy[0])), 5, [0, 0, 255], 2, 1)
                cv2.putText(frame, "Measured id: " + str(i), (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, [236, 252, 176])

                cv2.line(frame, (int(cpx[0]), int(cpy[0])),
                         (int(xk), int(yk)), (255, 51, 255), 1)
                cv2.putText(frame, "Predicted id: " + str(i), (int(cpx[0]), int(
                    cpy[0])), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [191, 144, 218])
                ...
                if obj == 1:
                    for j in range(len(contour_pointx)-1):
                        ...
                        cv2.circle(frame, (int(x_n), int(y_n)),
                                   1, [255, 51, 255], 2, 1)
                ...
                for j in range(0, future_state):
                    ...
                    current_measurement_future = np.array(
                        [[cpx_new], [cpy_new]])
                    ...
                    cv2.circle(frame, (int(cpnx[0]), int(
                        cpny[0])), 5, [255, 0, 0], 2, 1)

                    cv2.line(frame, (int(cpx_future[j]), int(cpy_future[j])), (int(
                        cpnx[0]), int(cpny[0])), (102, 255, 255), 1)
